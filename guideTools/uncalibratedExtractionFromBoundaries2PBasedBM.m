...
function [segmentsAcum,nAcum,calib] = uncalibratedExtractionFromBoundaries2PBasedBM(boundaries,subBounds,sizeSubBounds,kEnd,nAttempts,modelClass,lineExtractionParams,rows,u_0,v_0,plotFlag)
...
for j = 1:kEnd
    ...
    if modelClass == 2    
        disp('Not implemented yet for gradient: Changing to 3 points alg');
        ...
        [acceptedIndex, sizeAccepted, r_vlAcum] = uncalibratedLineExtractionDeviceDistance(xHat,xHat,gradThetaSel,nAttempts,shortPixelThreshold,getSign(modelClass),modelClass,rows); %% El ultimo campo permitiria el uso de fH si se conociese
...
%         fprintf('r_vlEst = %f, r_vlEstControl = %f, rVL = %f \n',r_vlEst,r_vlEstControl,rVL);        
        r_vlArray(j)=rVL;
    end
    %% End- Errors Control    
