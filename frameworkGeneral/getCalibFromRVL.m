...
function calib = getCalibFromRVL(r_vl,modelClass,fH)
% 1 ParamX - 2 Hyper - 3 Equiangular - 4 StereographicX - 5 Orthogonal - 6 Equisolid


switch(modelClass)
    case 1,               
        calib = r_vl;    % calib = r_vl = 2*f*p
    case 2,
        chi = atan(r_vl/fH);        
        calib = [fH;chi];
    case 3,        
        calib = r_vl*2/pi;  % calib = f = 2*r_vl/pi
    case 4,        
        calib = r_vl;   % calib = r_vl = 2*f
    case 5,        
        calib = r_vl;   %calib = f = r_vl 
    case 6,        
        calib = r_vl*sqrt(2);   %calib = f = r_vl*sqrt(2);
end