...
function [boundaries, boundary, sizeBoundaries]=getBoundaries(edges,mask)
...
sizeBoundaries = zeros(1,numel(boundaries));
for i=1:size(boundaries,1)
    boundary=[boundaries{i}(:,2)';boundaries{i}(:,1)'];
    boundary=[boundary;ones(1,size(boundary,2))];
    sub_boundaries{i}=boundary;
    sizeBoundaries(i)=size(boundary,2);
end;
