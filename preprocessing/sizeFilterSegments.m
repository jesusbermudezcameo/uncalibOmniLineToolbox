...
function upperI = sizeFilterSegments(sizeSegmentAcum,proportionalThres,infThres,upperThres) 


if ~exist('infThres','var')
    infThres = 0;
end
if ~exist('upperThres','var')
    upperThres = sizeSegmentAcum(end);
end

variableMinSize =  proportionalThres*sizeSegmentAcum(end);  

if variableMinSize < infThres
    sizeThreshold = infThres;
else if variableMinSize > upperThres
    sizeThreshold = upperThres;    
    else
        sizeThreshold = variableMinSize;        
    end
end

upperI = find(sizeSegmentAcum>sizeThreshold);
