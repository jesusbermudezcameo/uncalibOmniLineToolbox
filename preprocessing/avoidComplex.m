...
function [rVlCand nArray xAccepted nComplex] = avoidComplex(r_vlArray,nArray,xAccepted)

iNonComplex = getNonComplex(r_vlArray);

j1NonComplex = getNonComplex(nArray(1,:));
j2NonComplex = getNonComplex(nArray(2,:));
j3NonComplex = getNonComplex(nArray(3,:));

iNC = intersect(intersect(intersect(j1NonComplex,j2NonComplex),j3NonComplex),iNonComplex);

rVlCand = r_vlArray(iNC);
nArray = nArray(:,iNC);
xAccepted = xAccepted(iNC);

nComplex = numel(r_vlArray)-numel(iNC);
