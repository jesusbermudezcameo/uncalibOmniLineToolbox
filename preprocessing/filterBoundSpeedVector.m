...
function [binChanges,gradxF,gradyF] = filterBoundSpeedVector(bound,dx,dy,NUMdd,DENdd,threshold,mRows)

j = bound(1,:);
i = bound(2,:);

ii = coord2Index(i,j,mRows);
...
gradx = gradx./gradNorm;
grady = grady./gradNorm;


gradxF = filter(NUMdd,DENdd,gradx); 
gradyF = filter(NUMdd,DENdd,grady);
