...
function xHatParam=getParametricCurve(n,theta,calib,modelClass)

phiVarHat = atan2(-n(1),n(2));
phiHat=asin(n(3)/norm(n));

u = [cos(phiVarHat);sin(phiVarHat);0];
v = [-sin(phiHat)*sin(phiVarHat);sin(phiHat)*cos(phiVarHat);-cos(phiHat)];
X = [u v n]*[cos(theta);sin(theta);zeros(1,numel(theta))];

xHatParam = hModel(X,calib,modelClass); 
