...
function r_vl = getVanishingLine2DegHyper(xHat,dx,s,chi)
...
N = [[xHat(1,:); s*xHat(2,:);-cos(chi)*(cos(chi)-1)/sin(chi)^2*ones(1,numel(r)); -1/2*r.^2]    ,[-dx(2,:); s*dx(1,:); zeros(1,numel(r)) ;tau]]';
[~,~,V] = svd(N);
v = V(:,end);
