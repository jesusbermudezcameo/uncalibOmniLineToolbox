...
function [iAccepted, n] = testCandidateNoAngle(xSel,xTest,rTest,calib,modelClass,rThreshold)

sign=getSign(modelClass);

%% Computing normal 
n = nPointsToNormal(xSel,calib,modelClass); 

%% Computing Distance
alphaTest = getAlpha(rTest,calib,modelClass);
dist = abs([xTest(1,:);sign*xTest(2,:);-alphaTest]'*n);

%% Computing adaptative Threshold
grad = getGradient(n,xSel(1,:),xSel(2,:),calib,modelClass);
dxSel = normalizeVectorArray(grad);
algThr = getAlgThreshold(xSel,n,dxSel,rThreshold,calib,modelClass,sign);
